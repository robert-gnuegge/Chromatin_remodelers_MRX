...
\begin{document}
	...
	\begin{tikzpicture}[Label/.style={font=\bfseries, inner sep=0ex, xshift=1ex, anchor=west}]
		...
		%
		% Rad53 --------------------------------------
		\path (0,0) node[inner sep=0ex] (Rad53) {\includegraphics[width=\PicWidth]{Rad53_SrfI.jpg}};
		% frame
		...
		\begin{scope}[every node/.style={inner sep=0.5ex}]
			...
			\path (Rad53.north east) node[anchor=south west] (L) {\strut};
		...
		%
		% Pgk1 -----------------------------------
		\path (Rad53.south) ++(0,-1.5ex) node[inner sep=0ex, anchor=north] (Pgk1) {\includegraphics[width=\PicWidth]{Pgk1_SrfI.jpg}};
		% frame
