...
# read data ---------------------------------------------------------------
percent_input <- read.table(file = "03_Processed_data/percent_input_replicates.txt", header = TRUE)

# calc and save cutting mean, sd, and sample size -------------------------
percent_input_modes <- aggregate(mean ~ strain + primers + time, data = percent_input,
                                 FUN = function(x) c(mean = mean(x, na.rm = TRUE), sd = sd(x, na.rm = TRUE), n = 0))

# sort
percent_input_modes <- percent_input_modes[order(percent_input_modes$strain, percent_input_modes$primers, percent_input_modes$time),]
