...
\begin{document}
	...
	\begin{tikzpicture}[every node/.style={inner sep=0ex},%
		...
		%
		% YPD
		\path (0,0) node (YPD) {\includegraphics[width=\PicWidth]{YPD.jpg}};
		% strains
		\pgfmathsetmacro{\Start}{0.12}
		\pgfmathsetmacro{\Sep}{0.155}
		\foreach[count=\i] \strain in {WT, \textit{fun30$\Delta$}, \textit{fun30$\Delta$ exo1$\Delta$}, \textit{fun30$\Delta$ sgs1$\Delta$}, \textit{exo1$\Delta$ sgs1$\Delta$}, \textit{fun30$\Delta$ exo1$\Delta$ sgs1$\Delta$}}{
			\pgfmathsetmacro{\x}{\Start+(\i-1)*\Sep}
			\path ($(YPD.north west)!\x!(YPD.south west)$) node[xshift=-\LabelSep, anchor=east] {\strain};
		}
		%
		% CPT
		\path (YPD.east) ++(\HSep,0) node[anchor=west] (CPT) {\includegraphics[width=\PicWidth]{CPT.jpg}};
		%
		% add top Rap labels
		\path (YPD.north) ++(0,\LabelSep) node[anchor=south, MediaLabel] {YPD\vphantom{$\mu$g}};
		\path (CPT.north) ++(0,\LabelSep) node[anchor=south, MediaLabel] {YPD + 0.5 $\mu$g/ml CPT};
		%
	\end{tikzpicture}
	%
\end{document}