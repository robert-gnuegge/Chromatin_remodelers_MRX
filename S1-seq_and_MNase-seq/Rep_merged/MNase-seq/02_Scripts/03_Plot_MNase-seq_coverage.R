...
AT_genes <- AnnotationTrack(range = all_features[!(all_features$id == "") & all_features$type %in% c("ORF", "tRNA gene", "centromere", "ARS")], 
                            name = NULL, showFeatureId = TRUE, cex = 0.67, featureAnnotation = "id",
                            arrowHeadMaxWidth = 10, fill = "white", col = "gray", fontcolor.item = "gray")


# plotting ----------------------------------------------------------------
plot_dir <- "04_Plots/MNase-seq_coverage/"
...
roi <- resize(x = all_features[all_features$type == "ARS"][30], width = 2000, fix = "center")
