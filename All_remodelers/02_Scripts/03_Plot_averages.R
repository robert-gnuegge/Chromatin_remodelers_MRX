...
Legend_txt <- c("WT",
                ...
                expression(italic("ino80-aa")),
                expression(italic("chd1"*Delta)),
                expression(italic("swr1"*Delta)),
                expression(italic("isw1"*Delta)),
                expression(italic("isw2"*Delta)))

pdf(file = "tmp.pdf", width=1.10, height=2.05)
...
pdf(file = "tmp.pdf", width=3.1, height=0.85)
par(cex = 1, mar = rep(0, 4))
plot(1, type="n", axes=FALSE, xlab="", ylab="")
legend(1, 1, xjust=0.5, yjust=0.5, legend = Legend_txt, col = MyColors, pch = 20, ncol = 3)
dev.off()
